  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        <Link
          href="/"
          className="inline-flex items-center text-sm text-slate-400 hover:text-slate-200 mb-8 transition-colors"
        >
          <svg
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl backdrop-blur-sm">
          ...
          {video.status !== "COMPLETED" && video.status !== "PROCESSED" && (
            <div className="aspect-video bg-slate-950 rounded-xl flex flex-col items-center justify-center border border-dashed border-slate-800 mb-6 p-8 text-center">
              {video.status === "FAILED" ? (
                <div className="text-rose-500 text-lg font-semibold mb-2">
                  Processing Failed
                </div>
              ) : (
              ...
              <p className="text-sm text-slate-500 max-w-sm">
                We are generating variant qualities (360p, 480p, 720p). Refresh
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div
                className={`px-4 py-2 rounded-xl border ${currentStatusColor} font-medium text-sm`}
              >
                {(video.status === "PROCESSING" ||
                  video.status === "UPLOADED") && <p>Processing...</p>}
                {(video.status === "COMPLETED" ||
                  video.status === "PROCESSED") && <p>Ready</p>}
                {video.status === "FAILED" && <p>Failed</p>}
              </div>
            </div>
          </div>
        </div>
